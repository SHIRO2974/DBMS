...
select
-- partition by: 데이터를 그룹으로 나누는 기능 (ex 순번 1,2,3 (1번째 순서안에 1,2,3번 데이터))
...
select
	*,
    ...
    case 
        ...
        when category_id < 100 then '100 ~ 199'
        when category_id < 100 then '200 ~ 299'
        else '300 ~'
	end as scope,
-- ifnull: category_id, '40' id 값이 null 이라면 40으로 변경
    ifnull(category_id, '40') as ifnull,

-- nullif: category_id, '37' id가 37번 이라면 null값으로 변경
...
update
	book_tb
set
	category_id = ifnull(category_id, 4000);	-- category_id가 null이면 4000으로 변경
